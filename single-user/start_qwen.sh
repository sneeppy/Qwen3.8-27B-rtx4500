#!/bin/bash
# Qwen3.8-27B on a single 24 GB GPU — SINGLE USER / LOW LATENCY mode.
#
...
#   24 GB GPU under fp8+MTP+prefix caching at ~28-34k context. The flashinfer-free
...
# share of prefill. A WSL2 measured it 1-6% *negative* standalone (#62), so
...
if [ "$SPEC" = "dflash2" ]; then
  ...
  # reference 24 GB GPU, CTX=fast k=15: 2.66 tok/step and 101.4 tok/s unset against 3.23 and
  ...
  # (controlled 1-vs-two GPUs A/B on this harness):
  #
  ...
  if [ "$CTX" = "huge" ]; then
    ...
    # independent 24 GB GPU reported (PR #13). Raising the cudagraph reservation does NOT
    ...
    # 24 GB, which is what an independent 24 GB GPU report hit (PR #13).
    if [ "$DRAFT_TOKENS" -gt 7 ]; then
  ...
  # community drafters on 24 GiB: KV_MEM=3000000000 DFLASH_MAX_LEN=8192 (pinned, 24 GB GPU), or
  # GPU_UTIL=0.97 KV_MEM= DFLASH_MAX_LEN=8192 (unpinned, 24 GB GPU); the pinned profile costs the
  ...
  # allocator. Measured on one 24 GiB 24 GB GPU, SPEC=dflash2 k=7, free VRAM after boot
...
if [ "${VISION:-0}" = 1 ]; then
  ...
  # measured here, VISION=1 VISION_OFFLOAD=0 SPEC=dflash2, 24 GB GPU at 250 W. With the
